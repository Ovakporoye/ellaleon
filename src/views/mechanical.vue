<template>
  <div id="mechanical">
    <div class="top">
      <div class="inner">
        <div class="container py-5">
          <div class="fs-50 fw-6 text-white pt-5">Mechanical Engineering</div>
          <p class="text-white fs-20 ff-a mt-4">
            In Ellaleon Ventures Ltd, we ensure that we deliver world class
            services with adherence to the highest global practises, & safety
            standards. Our fabrication capabilities encompass Piping, civil
            structural, mechanical as shown below
          </p>
        </div>
      </div>
    </div>

    <div class="container">
      <div class="row">
        <div class="col-lg-6 order-1 order-lg-1">
          <div class="picture">
            <div class="s2">
              <div class="inner"></div>
            </div>
          </div>
        </div>
        <div class="col-lg-6 order-2 order-lg-2">
          <div class="text">
            <p class="fs-30 fw-7">Construction Management</p>
            <p class="fs-18">
              Our proven Construction Management capabilities combined with our
              experienced inspection services team provides total delivery on
              any project. We are experienced at and capable of executing
              projects offshore, at either domestic or international locations.
              Our Construction and installation team have expertise and
              management experience in the various offshore facilities as
              detailed below:
            </p>
            <div class="d-flex mb-1 align-content-center">
              <div class="mr-3">
                <img src="@/assets/img/minus.svg" width="24px" alt="" />
              </div>
              <div>
                <p class="fs-18">Production facilities</p>
              </div>
            </div>
            <div class="d-flex mb-1 align-content-center">
              <div class="mr-3">
                <img src="@/assets/img/minus.svg" width="24px" alt="" />
              </div>
              <div>
                <p class="fs-18">Power & Gas Plants</p>
              </div>
            </div>
            <div class="d-flex mb-1 align-content-center">
              <div class="mr-3">
                <img src="@/assets/img/minus.svg" width="24px" alt="" />
              </div>
              <div>
                <p class="fs-18">Pump & Compressor Stations</p>
              </div>
            </div>
          </div>
        </div>

        <div class="col-lg-6 order-4 order-lg-3">
          <div class="text">
            <p class="fs-30 fw-7">Offshore & Onshore Fabrication</p>

            <p class="fs-18 pr-lg-4">
              We have expertise in welding various steel including stainless,
              carbon and duplex using a wide range of processes.
            </p>
          </div>
        </div>
        <div class="col-lg-6 order-3 order-lg-4">
          <div class="picture">
            <div class="s2">
              <div class="inner"></div>
            </div>
          </div>
        </div>
      </div>
    </div>
  </div>
</template>